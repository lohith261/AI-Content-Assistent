...
// For Render, you'll need to set these as environment variables.
...
app.get('/generate-content', async (req, res) => {
    const { text, url, image, temperature, maxOutputTokens, userId } = req.query; // New: userId parameter
    let contentToProcess = '';
    let parts = [];

    if (image) {
        const [mimeType, base64Data] = image.split(';base64,');
        if (!mimeType || !base64Data) { return res.status(400).json({ error: "Invalid image data format." }); }
        ...
        try {
            ...
            if (!contentToProcess) { return res.status(400).json({ error: "Could not extract readable content from the provided URL." }); }
    ...
    try {
        ...
        const streamResult = await model.generateContent({ contents: [{ role: "user", parts: parts }], generationConfig: generationConfig, safetySettings: safetySettings }, { stream: true });
        ...
        // New: Save result to Firestore if userId is provided
        if (userId) {
            const entry = {
                input: text || url || 'Image Input', // Store the input type
                image: image || null, // Store image if present
...
app.get('/user-history', async (req, res) => {
    ...
    try {
        ...
        const snapshot = await historyRef.orderBy('timestamp', 'desc').limit(5).get(); // Get last 5, ordered by timestamp
